...
gmat = pd.concat(gmat)
meta = pd.concat(meta)
assert np.all(gmat.index.values == meta.index.values)

logging.info("before selecting hemi-brain sections {} {}".format(gmat.shape, meta.shape))

filter_cond = (
      ((meta['sample'] == 'Xulab_2_5_region_0') & (meta['x'] > 0))
    | ((meta['sample'] == 'Xulab_2_5_region_1') & (meta['x'] < 0))
    | ((meta['sample'] == 'Xulab_2_6_region_0') & (meta['x'] < 0))
    | ((meta['sample'] == 'Xulab_2_6_region_1') & (meta['x'] < 0))
)
gmat = gmat[filter_cond]
meta = meta[filter_cond]
logging.info("after selecting hemi-brain sections {} {}".format(gmat.shape, meta.shape))
